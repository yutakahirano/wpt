...
/**
 * Given a peer connection, return true after at least numFramesOrPackets
 * frames (video) or packets (audio) have been received. Otherwise return false
 * after 10 seconds.
 * @param {*} t Test instance.
...
async function waitForFrames(
  ...
  while (Date.now() < originalTimestampMs + kTimeoutMs) {
    ...
    if (!lookForAudio && !lookForVideo) return true;
...
/**
 * Given a peer connection, return true after one of its inbound RTP connections
 * includes use of the specified codec. Otherwise, return false after 10
 * seconds.
 * @param {*} t Test instance.
 * @param {!RTCPeerConnection} pc The peer connection.
